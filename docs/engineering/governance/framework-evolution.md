...
| v1.6 | F-030 First Execution Test | 2026-08-07 | **Execution completeness failure → implementation contract strengthened.** The first execution-milestone test (F-030 — User Display Name) validated the planning pipeline but FAILED on the primary behavior under test: the frontend implementation agent silently resolved two implementation-critical contract gaps (missing PUT response contract; missing display_name validation constraints), implemented both components, and deferred the required escalations. Findings: `docs/engineering/findings/F-030/F-030-execution-test-report.md` (Obs-F030-01..05). The observed failure demonstrated that the implementation agent could silently resolve an implementation-critical contract gap. **Actions (revised 2026-08-08):** (1) The corrective action is to strengthen **FI-I-003 — Implementation Completeness** (structured `NEEDS CLARIFICATION`; no project writes once an implementation-critical clarification is required) rather than introduce a separate workflow-level authorization gate. (2) Frontend implementation agent strengthened: Step 4 validates implementation completeness with structured `NEEDS CLARIFICATION` (Reason/Missing/Impact/Required); step budget 15→30. (3) Orphaned components archived to `docs/engineering/findings/F-030/evidence/`. (4) F-030 promoted to a permanent 3-rung execution regression benchmark (2 gaps → must clarify both; 1 → must clarify remaining; 0 → must implement). |
| v1.7 | Framework Simplification | 2026-08-08 | **Execution Authorization mechanism removed.** The separate workflow-level "Execution Authorization" mechanism (the FI-W-001 workflow invariant and the Pre-Implementation Authorization gate in `frontend-implementation-agent.md`) is removed. The F-030 failure's important requirement is not a new authorization subsystem but a stronger implementation-agent contract: an implementation agent must not silently resolve implementation-critical gaps in approved upstream artifacts. FI-I-003 (Implementation Completeness) is strengthened accordingly; the frontend implementation agent now returns structured `NEEDS CLARIFICATION` instead of an authorization verdict. No new workflow states, documents, gates, or agents were introduced. |
...
| Invariant | v1.0 | v1.1 | v1.2 | v1.3 | v1.4 | v1.5 | v1.6 | v1.7 |
|---|---|---|---|---|---|---|---|---|
| FI-I-001 | — | — | — | — | Proposed (as Reuse Evidence) | Proposed (renamed Evidence-Based Architecture) | Proposed | Proposed |
| FI-I-002 | — | — | — | — | — | Proposed | Proposed | Proposed |
| FI-I-003 | — | — | — | — | — | Proposed | Proposed | Proposed (strengthened) |
